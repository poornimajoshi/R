...
m <- cor(Filter(is.numeric,raw_data))
...
```{r }
data <- raw_data
data$Installs <- ifelse( data$Installs < 10000 , 0, data$Installs)
...
data = data[,-c(1, 2, 3,7,9,10,11,12, 13, 14, 15)]
str(data)
```
...
Interpretation
1. Model execution output shows some iteration history and includes the final negative log-likelihood 13920.747067. This value is multiplied by two as shown in the model summary as the Residual Deviance.
...
bpp2 = melt (bpp,id.vars=c("Rating","Reviews", "Size", "Type", "out"),value.name="probablity")
...
ggplot(bpp2, aes(x = Rating, y = probablity, colour = out)) +
